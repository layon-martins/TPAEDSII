...
Aptd_Pat InsereEntre(String S, Aptd_Pat *A, int i, int idDoc){
    ...
    if(EExterno(*A) || i < (*A)->NO.NInterno.Index){
        p = CriaNoExt(S,idDoc);
        if(EExterno(*A)){
            if(ComparaLetra(i, (*A)->NO.Chave[i], S) == 1)return(CriaNoInt(i, A, &p, (*A)->NO.Chave[i]));
            return (CriaNoInt(i, &p, A, S[i]));
        }
...
Aptd_Pat InserePatricia(String S, Aptd_Pat *t, int idDoc){
    ...
    while(!EExterno(*p)){
        if(strlen(S) < (*p)->NO.NInterno.Index)p = AchaNoExterno(p);
            ...
            if(ComparaLetra((*p)->NO.NInterno.Index, (*p)->NO.NInterno.Caractere, S))p = &(*p)->NO.NInterno.Dir;
...
Aptd_Pat Pesquisa(String S, Aptd_Pat A){
    if(EExterno(A)){
        if(strcmp(S, A->NO.Chave))return NULL;
        return A;
    }
    if(ComparaLetra(A->NO.NInterno.Index, A->NO.NInterno.Caractere, S))return Pesquisa(S, A->NO.NInterno.Dir);
